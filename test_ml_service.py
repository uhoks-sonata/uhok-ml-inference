...
async def test_single_embedding(client: httpx.AsyncClient, text: str) -> Dict[str, Any]:
    """단일 텍스트 임베딩 테스트"""
    ...
    try:
        ...
        return {
            ...
            "embedding_preview": data["embedding"][:5]
        }
    except Exception as e:
        print(f"❌ 임베딩 실패: {e}")
        return {"success": False, "error": str(e)}

async def test_batch_embedding(client: httpx.AsyncClient, texts: list) -> Dict[str, Any]:
    """배치 임베딩 테스트"""
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        # 2. 모델 정보 조회
        await test_model_info(client)
        print()

        # 3. 단일 임베딩 테스트
        single_results = []
        for text in TEST_TEXTS[:3]:
            result = await test_single_embedding(client, text)
            single_results.append(result)
            print()

        # 4. 배치 임베딩 테스트
        batch_result = await test_batch_embedding(client, TEST_TEXTS)
        print()
